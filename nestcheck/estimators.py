...
def get_named_estimator(func_in, **kwargs):
    if kwargs:
        estimator = func_in
    else:
        estimator = functools.partial(func_in, **kwargs)
...
def get_latex_name(func, **kwargs):
    ...
    if func.__name__ == 'logz':
        ...
        latex_name = r'$\mathrm{C.I.}_{' + percent_str + r'\%}(|\theta|)$'
    elif func.__name__ == 'param_mean':
        latex_name = (r'$\overline{\theta_{\hat{' + str(param_ind + 1) +
                      '}}}$')
    elif func.__name__ == 'param_squared_mean':
        param_ind = kwargs.pop('param_ind', 0)
        latex_name = (r'$\overline{\theta^2_{' + str(param_ind + 1) + '}}$')
    elif func.__name__ == 'param_cred':
        param_str = r'\theta_{\hat{' + str(param_ind + 1) + '}}'
        if probability == 0.5:
            latex_name = r'$\mathrm{median}(' + param_str + ')$'
        else:
            # format percent without trailing zeros
            percent_str = ('%f' % (probability * 100)).rstrip('0').rstrip('.')
            latex_name = (r'$\mathrm{C.I.}_{' + percent_str +
                          r'\%}(' + param_str + ')$')
    else:
